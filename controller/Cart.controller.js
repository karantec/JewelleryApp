...
exports.addToCart = async (req, res) => {
  try {
    ...
    const product = await GoldProduct.findById(productIdd);
    if (!product) {
    ...
    let cart = await Cart.findOne({ userIdd });
    if (!cart) {
      cart = new Cart({ userIdd, items: [] });
    }

    const existingItem = cart.items.find(
      (item) => item.productId.toString() === productIdd
    );
    if (existingItem) {
